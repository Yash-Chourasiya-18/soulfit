      ...
      if (config && config.smtpPass) {
        ...
        if (targetEmail) {
          await transporter.sendMail({
            ...
            to: targetEmail,
            subject: `Ghar Saaj - ${type.replace(/_/g, ' ')}`,
            text: message,
            html: `
              <div style="font-family: sans-serif; padding: 20px; color: #333;">
                <h2 style="color: #000;">Ghar Saaj Notification</h2>
                <p>${message}</p>
                <hr style="border: 0; border-top: 1px solid #eee; margin: 20px 0;" />
                <small style="color: #999;">This is an automated message from your Ghar Saaj store.</small>
              </div>
            `
          });
        }
